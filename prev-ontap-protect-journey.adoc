...
---
= NetApp Backup and Recovery 中的保護旅程規劃選項
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
快照是所有備份方法的基礎，是使用備份和還原服務的必要條件。快照是磁碟區的唯讀、時間點影像。此映像佔用極少的儲存空間，由於它只記錄自上次快照建立以來檔案發生的更改，因此效能開銷可以忽略不計。在磁碟區上建立的快照用於保持複製磁碟區和備份檔案與來源磁碟區所做的變更保持同步—如圖所示。
...
除了這些標準之外，雲端儲存如果使用 *Datalock and Ransomware protection* 功能，還能提供額外的安全選項，並且透過為較舊的備份檔案選擇歸檔儲存類別來節省更多成本。link:prev-ontap-policy-object-options.html["了解有關 DataLock 和勒索軟體保護以及檔案儲存設定的更多信息"].
