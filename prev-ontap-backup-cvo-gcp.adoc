...
---
= 使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 備份至 Google Cloud Storage
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 系統中的 Volume 資料備份到 Google Cloud Storage。
...
您還需要一個 Google Cloud 帳戶，用於存放備份的儲存空間。
...
若要使用 NetApp 備份與還原的搜尋與還原功能，Console 代理程式的角色必須包含特定權限，以便其能夠存取 Google Cloud BigQuery 服務。請參閱下列權限，如果需要修改原則，請依照步驟操作。
...
. *詳細資料和憑證*：輸入以下資訊：
+
.. 選擇 *Edit Project*，如果要使用的專案與預設專案（Console 代理程式所在的專案）不同，請選擇新專案。
.. 指定叢集名稱。
...
. *服務*：保持 NetApp Backup and Recovery 啟用，然後選擇 *繼續*。
...
.. 在 *服務帳戶的存取金鑰* 下，選取 *為服務帳戶建立金鑰*，選取您剛建立的服務帳戶，然後選取 *建立金鑰*。
+
...
+
如果您的備份 Google Cloud Storage 目的地已在 Console 的 *Systems* 頁面上作為系統存在，則可以將 ONTAP 叢集拖曳到 Google Cloud Storage 系統上。
...
** 輸入策略的名稱。
** 選擇最多五個時間表，通常頻率不同。
** 對於備份至物件原則，請設定 Datalock 和勒索軟體保護。如需 Datalock 和勒索軟體保護的詳細資訊，請參閱 link:prev-ontap-policy-object-options.html["備份到對象策略設置"]。
** 選擇“*創建*”。
