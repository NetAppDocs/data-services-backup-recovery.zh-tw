...
[role="lead"]
ONTAP磁碟區資料的備份可以從您建立備份的位置取得：快照、複製磁碟區和儲存在物件儲存中的備份。您可以從這些備份位置中的任何一個還原特定時間點的資料。使用NetApp Backup and Recovery，可以從備份文件還原整個ONTAP卷，或者如果只需要還原幾個文件，則可以還原資料夾或單一文件。
...
在復原之前，請記下來源磁碟區名稱、來源系統和 SVM 以及備份檔案日期。您可以從快照、複製磁碟區或儲存在物件儲存中的備份還原ONTAP資料。


NOTE: 如果您要復原的備份檔案位於歸檔雲端儲存中（從ONTAP 9.10.1 開始），則復原作業需要更長時間並會產生費用。對於磁碟區恢復，目標叢集必須執行ONTAP 9.10.1 或更高版本；對於檔案恢復，目標叢集必須執行 ONTAP 9.11.1 或更高版本；對於 Google Archive 和StorageGRID，目標叢集必須執行 ONTAP 9.12.1 或更高版本；對於資料夾恢復，目標必須執行 ONTAP 9.13.13.13.13.
...
. 若要從磁碟區備份中還原資料夾或文件，請選擇「還原」選單，然後在「瀏覽與還原」下選擇「復原檔案或資料夾」。
...
搜尋操作會尋找ONTAP磁碟區的所有本機快照、輔助儲存系統上的所有複製磁碟區以及物件儲存中存在的所有備份檔案。由於從本機快照或複製磁碟區還原資料比從物件儲存中的備份檔案還原資料速度更快、成本更低，因此您可能需要從這些其他位置還原資料。
