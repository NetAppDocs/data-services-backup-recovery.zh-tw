...
[role="lead"]
透過NetApp Backup and Recovery，您可以透過變更備份計畫、啟用/停用磁碟區備份、暫停備份、刪除備份、強制刪除備份等方式管理Cloud Volumes ONTAP和本機ONTAP系統的備份。這包括所有類型的備份，包括快照、複製磁碟區和物件儲存中的備份檔案。您也可以取消註冊NetApp Backup and Recovery。
...
. 查看「現有保護」欄位中保護選項的狀態。這 3 個圖示分別代表「本機快照」、「複製磁碟區」和「物件儲存中的備份」。
+
...
.步驟
. 在「卷」標籤中，找到要啟動備份的捲，然後選擇「操作」選單。 image:icon-action.png["操作圖示"] 在行尾，選擇「啟動 3-2-1 保護」。
. 在「定義備份策略」頁面中，選擇備份架構，然後定義本機快照、複製磁碟區和備份檔案的策略和其他詳細資訊。查看您在此系統中啟動的初始磁碟區的備份選項的詳細資訊。然後選擇“下一步”。
. 檢查此卷的備份設置，然後選擇*啟動備份*。
...
+
如果在為該叢集啟動 NetApp Backup and Recovery 時，在初始備份原則中為雲端備份啟用了 _Datalock 和 Ransomware protection_，則只會看到已設定 DataLock 的其他原則。如果在啟動 NetApp Backup and Recovery 時未啟用 _Datalock 和 Ransomware protection_，則只會看到未設定 DataLock 的其他雲端備份原則。
...
如果您在為此叢集啟用 NetApp Backup and Recovery 時啟用了 _Datalock and Ransomware protection_，隨需備份也會配置 DataLock，且保留期間為 30 天。隨機備份不支援勒索軟體掃描。link:prev-ontap-policy-object-options.html["了解有關 DataLock 和勒索軟體保護的更多信息"^]。
...
只有當磁碟區備份是從 ONTAP 9.11.1 或更高版本的系統建立，並且在備份至物件原則中啟用了 _Datalock 和 Ransomware 保護_ 時，此功能才可用。
...
[NOTE]
====
* 如果在為該叢集啟動 NetApp Backup and Recovery 時，初始原則中啟用了_資料鎖定和勒索軟體保護_，則之後編輯的任何原則都必須配置相同的 DataLock 設定（治理或合規性）。如果在啟動 NetApp Backup and Recovery 時未啟用_資料鎖定和勒索軟體保護_，則現在無法啟用 DataLock。
...
[NOTE]
====
* 如果您在啟用此叢集的 NetApp Backup and Recovery 時於初始原則中啟用了 _Datalock and Ransomware protection_，則您建立的任何其他原則都必須設定為相同的 DataLock 設定（Governance 或 Compliance）。如果您在啟用 NetApp Backup and Recovery 時未啟用 _Datalock and Ransomware protection_，則無法建立使用 DataLock 的新原則。
...
.步驟
. 從 *Volumes* 功能表中，按一下來源磁碟區的 image:icon-actions-horizontal.gif["更多圖標"]，然後選取 *Manage backups > View backup*。
+
顯示所有備份檔案的清單。
