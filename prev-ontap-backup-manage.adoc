...
[role="lead"]
使用 NetApp Backup and Recovery 管理 Cloud Volumes ONTAP 和內部部署 ONTAP 系統的快照、複寫和物件儲存備份。您可以變更排程、啟用或停用 Volume 備份、暫停備份、刪除備份以及取消註冊服務。
...
您可以在磁碟區備份儀表板中查看目前正在備份的所有磁碟區的清單。這包括所有類型的備份，包括快照、複製磁碟區和物件儲存中的備份檔案。您也可以查看系統中目前未備份的磁碟區。
...
. 請查看「現有保護」欄中的保護選項狀態。這三個圖示分別代表「本機快照」、「複寫 Volume」和「物件儲存中的備份」。
+
當此備份類型啟動時，對應的圖示會亮起；當該備份類型未啟動時，圖示會變為灰色。您可以將遊標停留在每個圖示上，查看正在使用的備份策略以及每種備份類型的其他相關資訊。
...
如需相關指示，請參閱 link:br-start-enable.html["啟用 NetApp Backup and Recovery 以保護磁碟區"^]。
...
您可以變更指派給已指派原則的現有磁碟區的備份策略。您可以變更本機快照、複製磁碟區和備份檔案的策略。您想要套用於磁碟區的任何新快照、複製或備份策略都必須已經存在。
...
.步驟
. 從「卷」選單中，找到要修改策略設定的捲，然後選擇「操作」選單。 image:icon-action.png["操作圖示"] 在行尾，選擇「編輯備份策略」。
. 在「編輯備份策略」頁面中，對本機快照、複製磁碟區和備份檔案的現有備份策略進行更改，然後選擇「下一步」。
+
如果在為該叢集啟動 NetApp Backup and Recovery 時，在初始備份原則中為雲端備份啟用了 _DataLock 與勒索軟體保護_，則只會看到已設定 DataLock 的其他原則。如果在啟動 NetApp Backup and Recovery 時未啟用 _DataLock 與勒索軟體保護_，則只會看到未設定 DataLock 的其他雲端備份原則。
...
當啟用備份的磁碟區超過五個時， NetApp Backup and Recovery一次只會初始化五個磁碟區。當這些完成後，它會以 5 個為一組繼續進行，直到所有捲都初始化完畢。

.步驟
. 從「*卷*」標籤中，按磁碟區所在的系統進行篩選。
. 選擇您想要管理備份設定的所有磁碟區。
. 根據您要設定的備份行動類型，在「批次作業」功能表中選擇對應的按鈕：
+
[cols="50,30"]
|===
| 備份作業... | 選擇此按鈕... 
...
|===
. 在出現的備份頁面中，對本機快照、複製磁碟區或備份檔案的現有備份策略進行更改，然後選擇「儲存」。
+
如果在為該叢集啟動 NetApp Backup and Recovery 時，在初始備份原則中為雲端備份啟用了 _DataLock 與勒索軟體保護_，則只會看到已設定 DataLock 的其他原則。如果在啟動 NetApp Backup and Recovery 時未啟用 _DataLock 與勒索軟體保護_，則只會看到未設定 DataLock 的其他雲端備份原則。
...
您可以建立磁碟區的臨時快照或備份到物件儲存。您無法建立臨時複製磁碟區。

備份名稱包含時間戳，以便您可以從其他計畫備份中識別出您的按需備份。

如果在為此叢集啟動 NetApp Backup and Recovery 時啟用了「DataLock 與勒索軟體保護」，則隨需備份也將設定 DataLock，且保留期間為 30 天。臨機操作備份不支援勒索軟體掃描。link:prev-ontap-policy-object-options.html["了解有關 DataLock 和勒索軟體保護的更多信息"^]。

建立臨時備份時，會在來源 Volume 上建立快照。由於此快照不屬於一般快照排程，因此不會自動輪換刪除。備份完成後，您可能需要手動從來源 Volume 中刪除此快照。這將釋放與此快照相關的區塊。快照名稱將以 `cbs-snapshot-adhoc-`. https://docs.netapp.com/us-en/ontap/san-admin/delete-all-existing-snapshot-copies-volume-task.html["了解如何使用 ONTAP CLI 刪除快照"^]開頭。
...
只有當 Volume 備份是從搭載 ONTAP 9.11.1 或更高版本的系統建立，且您在備份至物件原則中啟用了 _DataLock 和勒索軟體保護_ 時，此功能才可使用。
...
. 選擇 image:icon-actions-horizontal.gif["操作圖示"] 以取得您要掃描勒索軟體的 Volume 備份檔案，然後選擇*掃描勒索軟體*。
+
勒索軟體保護欄顯示掃描正在進行中。
...
| 暫停複製 | 暫停快照的增量傳輸，以更新目標磁碟區。如果您想重新開始增量更新，可以稍後再恢復。 
...
[NOTE]
====
* 如果您在為此叢集啟動 NetApp Backup and Recovery 時，已在初始原則中啟用「_DataLock 與勒索軟體保護_」，則您編輯的任何原則都必須設定相同的 DataLock 設定（監管或法規遵循）。如果您在啟動 NetApp Backup and Recovery 時未啟用「_DataLock 與勒索軟體保護_」，則現在無法啟用 DataLock。
...
+
link:prev-reference-aws-archive-storage-tiers.html["了解有關使用 AWS 檔案儲存的更多信息"]。link:prev-reference-azure-archive-storage-tiers.html["了解有關使用 Azure 檔案儲存的詳細信息"]。link:prev-reference-gcp-archive-storage-tiers.html["詳細了解如何使用 Google 歸檔存儲"]。（需ONTAP 9.12.1。）

+
請注意，如果您停止將備份分層到歸檔存儲，則任何已分層到歸檔存儲的備份文件仍將保留在該層中 - 它們不會自動移回標準層。只有新建立的磁碟區備份才會位於標準圖層。
...
[NOTE]
====
* 如果您在為此叢集啟動 NetApp Backup and Recovery 時，已在初始原則中啟用「_DataLock 與勒索軟體保護_」，則您建立的任何其他原則都必須設定相同的 DataLock 設定（監管或法規遵循）。如果您在啟動 NetApp Backup and Recovery 時未啟用「_DataLock 與勒索軟體保護_」，則無法建立使用 DataLock 的新原則。
...
請注意，此操作不會影響快照或複製磁碟區—這些類型的備份檔案不會被刪除。
...
NOTE: 備份關係刪除後、您將無法刪除個別磁碟區備份檔案。在刪除磁碟區的備份關係之前、請先刪除該磁碟區的備份檔案。

.步驟
. 從 *Volumes* 選單中，選擇來源 Volume 的 image:icon-actions-horizontal.gif["更多圖標"]，然後選擇 *管理備份 > 檢視備份*。
+
顯示所有備份檔案的清單。

. 從清單中選擇一個備份，然後選擇 *Actions > Delete all Backups*。
. 輸入磁碟區名稱以確認刪除。
. 展開 *Advanced settings* 區域。
. 在 *強制刪除備份* 下，選擇是否強制刪除所有備份。
+
在少數情況下，您可能需要撤銷 NetApp Backup and Recovery 的備份存取權；例如，當 Backup and Recovery 失去對備份儲存區的存取權，或備份受到 DataLock 保護但您不再需要這些備份時。使用此選項可在磁碟區和系統層級強制刪除備份。

+

CAUTION: 僅在需要徹底清理時才使用此選項。啟用後，NetApp Backup and Recovery 將不再存取這些備份（即使物件仍保留在雲端儲存中）。您必須手動從雲端服務提供者刪除它們。
...
不再需要個別備份時，可以刪除。您無法刪除複寫的磁碟區（資料保護磁碟區）。


NOTE: 備份關係刪除後，您無法刪除個別磁碟區備份檔案。但是，您可以<<刪除卷的所有備份文件,刪除該磁碟區的所有備份檔案>>。

.步驟
. 從「卷」標籤中選擇image:icon-actions-horizontal.gif["更多圖標"]對於來源磁碟區並選擇*查看磁碟區詳細資料*。
+
顯示卷的詳細信息，您可以選擇*快照*、*複製*或*備份*來查看該卷的所有備份文件的列表。預設情況下，會顯示可用的快照。
...
NOTE: 備份關係刪除後，您將無法刪除單一磁碟區備份檔案。但是，您可以<<刪除卷的所有備份文件,刪除該磁碟區的所有備份檔案>>。如果您需要刪除某個磁碟區的備份關係，請務必先刪除該磁碟區的備份，然後再刪除備份關係。
...
停用 NetApp Backup and Recovery 會暫停該系統所有磁碟區的備份與還原，但不會刪除現有的備份。這可讓您在不從此系統註銷備份服務的情況下，暫停所有備份與還原活動一段時間。
