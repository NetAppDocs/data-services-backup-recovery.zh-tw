...
---
= 使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 備份至 Azure Blob 儲存設備
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 系統中的 Volume 資料備份到 Azure Blob 儲存設備。
...
若要使用 NetApp Backup and Recovery 搜尋與還原功能，Console 代理程式的角色必須包含特定權限，以便其能夠存取 Azure Synapse 工作區和 Azure Data Lake Storage 帳戶。請參閱下列權限，如果需要修改原則，請依照步驟操作。
...
.步驟
. 在 Console 的 *Systems* 頁面中，選取系統，然後在右側面板的備份與還原旁邊選取 *Enable*。
+
...
選擇要保護的 Volume。受保護的 Volume 是指具有下列一項或多項原則的 Volume：快照原則、複寫原則、備份至物件原則。
...
NOTE: 如果您選擇的是現有的 Azure 儲存設備帳戶，加密資訊已可使用，因此您現在無需輸入。
...
.結果
NetApp Backup and Recovery開始對您的磁碟區進行初始備份。複製捲和備份檔案的基線傳輸包括主儲存系統資料的完整副本。後續傳輸包含快照中包含的主儲存資料的差異副本。
