...
---
= SnapCenter與NetApp Backup and Recovery之間的策略差異
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
使用此參考資料了解 SnapCenter 與 NetApp Backup and Recovery 之間的原則差異，這些差異可能會影響匯入的資源和原則。
...
NetApp Backup and Recovery 使用以下幾種排程分層，各層級略有不同：
...
NetApp Backup and Recovery 會依照下列方式調整 SnapCenter 排程：
...
* 如果 SnapCenter 的排程設定超過 7 天，NetApp Backup and Recovery 會將排程設為每月一次。在這個月內，部分快照將會被略過。
+
...
* 範例 *：如果您有 SnapCenter 每小時原則，從上午 1:00 開始每 5 小時重複一次，NetApp Backup and Recovery（移轉後）會將排程設定為每日，間隔 5 小時，時間為上午 1:00、上午 6:00、上午 11:00、下午 4:00 和晚上 9:00。某些時段會被跳過，晚上 9:00 之後應該是上午 2:00 才能每 5 小時重複一次，但始終會是上午 1:00。
