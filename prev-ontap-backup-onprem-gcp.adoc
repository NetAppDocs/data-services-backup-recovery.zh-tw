...
[role="lead"]
在 NetApp 備份與還原中，您可以將內部部署 ONTAP 系統的 Volume 資料備份至次要儲存設備和 Google Cloud Storage。請使用此頁面準備您的環境並啟動備份。
...
* 請注意、如果您使用的 IPspace 與 Default 不同、則可能需要建立靜態路由才能存取物件儲存設備。
...
+
如果您的備份目的地 Google Cloud Storage 存在於 NetApp Console 的「系統」頁面上，您可以將 ONTAP 叢集拖曳到 Google Cloud Storage 上。
...
** *加密金鑰*：如果您建立了一個新的 Google Cloud Storage 儲存桶，請輸入供應商提供給您的加密金鑰資訊。您可以選擇使用預設的 Google Cloud 加密金鑰，或從您的 Google Cloud 帳戶中選擇您自己的客戶管理金鑰，以管理資料的加密。
+

NOTE: 如果您選擇的是現有的 Google Cloud Storage 儲存桶，則加密資訊已可使用，因此您現在無需輸入。
