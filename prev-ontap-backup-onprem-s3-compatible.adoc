...
====
.步驟
請注意，如果您選擇的 Volume 已套用快照或複寫原則，則您稍後選擇的原則將覆寫這些現有原則。
...
TIP: 如果要在啟動快照之前建立自訂原則，可以使用 System Manager 或 ONTAP CLI `snapmirror policy create`命令。請參閱。
...
** *將現有快照匯出至物件儲存作為備份檔案*：如果此系統中存在與您剛剛選擇的備份排程標籤（例如，每日、每週等）相符的 Volume 的本機快照，則會顯示此附加提示。勾選此核取方塊，即可將所有歷史快照複製到物件儲存作為備份檔案，以確保您的 Volume 獲得最完整的保護。
...
.步驟
. 在「審核」頁面中，審核您的選擇。
. 您可以選擇勾選「自動將快照原則標籤與複寫和備份原則標籤同步」複選框。這樣，建立的快照標籤將與複寫和備份原則中的標籤相符。如果原則不相符，則不會建立備份。
. 選擇*啟動備份*。
