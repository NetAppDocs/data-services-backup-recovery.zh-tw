...
服務費支付給NetApp ，涵蓋建立備份到物件儲存或ONTAP目標的成本，以及從這些備份還原磁碟區或檔案的成本。您只需為在物件儲存或ONTAP中保護的資料付費（這包括SnapMirror配置）。費用是根據備份到物件儲存或ONTAP目標的ONTAP磁碟區的來源邏輯已使用容量（在ONTAP效率之前）計算的。此容量也稱為前端兆位元組 (FETB)。
