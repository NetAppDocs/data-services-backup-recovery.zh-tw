...
. *主機註冊*：新增 Oracle Database 主機。在 *Host FQDN or IP address* 欄位中，輸入主機的 FQDN 或 IP 位址 - 對於叢集資料庫，您可以輸入叢集中任何節點的 FQDN 或 IP。然後提供認證資料、NetApp Console 代理程式和連接埠號碼。
...
[NOTE]
====
NetApp Backup and Recovery 透過 SSH 連線至每個主機，以自動安裝外掛程式。當出現以下任一情況時，請啟用*使用手動安裝*：
...
TIP: 新增資源可能需要幾分鐘。若要查看進度，請在 Inventory 頁面底部的狀態對話方塊中選取 *Track progress* ，或從左側選單中選取 *Monitoring* 。
