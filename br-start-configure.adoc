...
* <<為主機資源新增憑證>> 適用於任何 Windows、Microsoft SQL Server、Oracle Database 或 Linux 主機，NetApp Backup and Recovery 需要進行驗證。這包括在還原檔案或資料夾時所使用的來賓作業系統認證。
...
. 選擇“*憑證*”的向下箭頭。
. 選擇*新增憑證*。
. 請輸入登入憑證資訊。根據您選擇的身份驗證模式，會顯示不同的欄位。將滑鼠懸停在資訊 *i* 圖示上，以查看有關欄位的更多資訊。
+
** *憑證名稱*：輸入憑證的名稱。
** *驗證模式*：選擇 *Windows*、*Microsoft SQL*、*Oracle Database* 或 *Linux*。
+

TIP: 對於 Microsoft SQL Server 工作負載，您需要輸入 Windows 和 Microsoft SQL Server 的憑證，因此您需要新增兩組憑證。
...
如果您使用 Apache CloudStack 管理平台來管理 KVM 資源，則需要將其與NetApp Backup and Recovery集成，以便 Backup and Recovery 可以發現和保護受管 KVM 主機和虛擬機器。
...
建立資源轉換範本，用於儲存您在還原資源時對資源所做的常見轉換。

.步驟
. 從NetApp Backup and Recovery選單中，選擇 *設定*。
. 選擇向下箭頭以展開 *Resource modifer template* 區段。
. 選擇 *Create template* 。
. 輸入資源轉換範本的名稱。
. （選用）輸入範本說明。
. 描述目標資源：
