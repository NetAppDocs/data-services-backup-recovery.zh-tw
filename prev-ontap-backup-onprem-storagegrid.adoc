...
[role="lead"]
在 NetApp Backup and Recovery 中，您可以將主要內部部署 ONTAP 系統的 Volume 資料備份至次要儲存設備和 StorageGRID 物件儲存。請使用此頁面準備您的環境並啟動備份。
...
+
新版 NetApp Backup and Recovery 除了支援建立物件儲存備份外，還新增了排程和建立快照及複寫 Volume 的功能，但需要使用 3.9.31 或更高版本的 Console 代理程式。NetApp 建議您使用此最新版本來管理所有備份。
...
有關 StorageGRID 的 DataLock 和勒索軟體保護要求的詳細資訊，請參閱 link:prev-ontap-policy-object-options.html["備份到物件策略選項"]。

支援的StorageGRID版本:: 支援StorageGRID 10.3 及更高版本。
+
--
若要使用 DataLock 與勒索軟體保護來保護您的備份、您的 StorageGRID 系統必須執行 11.6.0.3 或更新版本。

要將舊備份分層到雲端檔案存儲，您的StorageGRID系統必須運行 11.3 或更高版本。此外，您的StorageGRID系統必須在控制台*系統*頁面上被發現。

若要使用歸檔儲存設備，需要管理節點 IP 存取。
...
+
如果您的叢集使用的是 ONTAP 9.11.1 或更高版本，您可以選擇設定 _DataLock and Ransomware protection_ 來保護備份檔案免於刪除和勒索軟體攻擊。_DataLock_ 可防止備份檔案遭到修改或刪除，_ransomware protection_ 則掃描備份檔案，尋找備份檔案中勒索軟體攻擊的跡象。
