...
[role="lead"]
利用這些資訊，您可以了解 NetApp Backup and Recovery 支援的 Azure 歸檔層。您也可以審查已歸檔備份的還原成本和擷取時間。
...
備份檔案最初建立時，會儲存在「冷存取分層」。該分層針對儲存不經常存取的資料進行了最佳化，但需要時可以立即存取。
...
NOTE: 從 Azure 還原資料到 StorageGRID 系統時，不支援高優先權。
