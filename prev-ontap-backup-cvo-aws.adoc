...
---
= 使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 備份至 Amazon S3
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 系統中的 Volume 資料備份到 Amazon S3。
...
NOTE: 如果您使用私有 VPC 端點進行備份或還原資料傳輸，則其優先權高於任何已設定的 Proxy 連線。
...
您可以在新建或現有的 Cloud Volumes ONTAP 系統上啟用 NetApp Backup and Recovery。不同選項的步驟略有不同。
...
.步驟
. 在 NetApp Console 的 *Systems* 頁面中，選取叢集，然後在右側面板的 Backup and Recovery 旁邊選取 *Enable*。
+
如果您備份的 Amazon S3 目的地以系統形式存在於「系統」頁面上，您可以將叢集拖曳到 Amazon S3 系統上以啟動設定精靈。
...
您可以隨時直接從 Cloud Volumes ONTAP 系統啟動備份。
...
+
如果您的備份 Amazon S3 目的地作為系統存在於 Console *Systems* 頁面上，您可以將 ONTAP 叢集拖曳到 Amazon S3 系統上。

** 在 Backup and Recovery 列中選取 *Volumes*。在 Volumes 索引標籤中，選取 *Actions* image:icon-action.png["操作圖示"] 圖示，然後針對單一 Volume （尚未啟用複寫或備份至物件儲存）選取 *Activate 3-2-1 Protection*。


+
精靈的「簡介」頁面顯示了保護選項，包括本機快照、複寫和備份。如果您在此步驟中使用了第二個選項，則會顯示「定義備份策略」頁面，並預設選擇一個 Volume。
...
.步驟
. 在「審核」頁面中，審核您的選擇。
. 您可以選擇勾選「自動修復本機快照、複寫和備份中的標籤不符」複選框。這將建立標籤與快照、複寫和備份原則中的標籤相符的快照。
. 選擇*啟動備份*。
