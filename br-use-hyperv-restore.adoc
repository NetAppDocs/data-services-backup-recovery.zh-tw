...
從「還原」頁面、您可以還原資源、即使您不記得確切名稱、資源所在位置、或資源上次處於良好狀態的日期。您可以使用篩選器搜尋快照。
...
.. 在「目標儲存設備」部分（僅從物件存放區還原時可見）：
+
*** *變更預設儲存設備位置*：啟用此選項可在來源儲存設備位置無法存取或已滿時，將物件存放區備份還原到其他位置。選擇要使用的其他叢集、儲存設備虛擬機器或彙總。
