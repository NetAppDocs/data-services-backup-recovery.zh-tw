...
*必要的 NetApp Console 角色* Backup and Recovery 超級管理員、Backup and Recovery 備份管理員、Backup and Recovery 還原管理員、Backup and Recovery Clone 管理員或 Backup and Recovery 備份檢視器角色。如果您要保護儲存在 Cloud Volumes ONTAP 叢集上的資料，則還需要 Cloud Volumes ONTAP 管理員角色。了解更多 link:reference-roles.html["備份與恢復角色及權限"]。 https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["了解所有服務的NetApp Console存取角色"^]。
