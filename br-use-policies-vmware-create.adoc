...
---
= 在 NetApp Backup and Recovery 中建立及管理 VMware 備份原則
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
使用 NetApp Backup and Recovery 建立 VMware 備份原則，以控制備份頻率、排程和保留。
...
NOTE: 對於 VMware 工作負載，備份計畫使用 VMware vCenter 伺服器的時區，而不是 UTC 或瀏覽器的本機時間。此時區在您建立排程時設定。如果主機時區發生變更，您必須重新整理主機並重新建立排程，才能使新時區生效。如需詳細資訊，請參閱 link:br-use-vmware-protection-groups.html["為 VMware 工作負載建立和管理保護群組"]。
...
+
對於 VMware 工作負載，此操作會在主儲存系統上的資料儲存區或虛擬機器上設定本機快照，然後將資料從主磁碟儲存設備系統複寫至次要磁碟儲存設備系統。
...
+
對於 VMware 工作負載，這會在主要資料存放區或虛擬機器上設定本機快照，並將資料從主磁碟儲存設備複寫到雲端物件儲存。
...
** *VM 設定*：
+
*** *啟用 VM 一致性快照和應用程式一致性快照*：啟用此選項可建立 VM 一致性快照和應用程式一致性快照。這要求在 VM 上執行 VMware Tools。如果 VMware Tools 未執行，則快照將改為損毀狀況一致快照。請注意，啟用此選項可能會增加備份時間並使用更多儲存設備。此外，VM 的作用中 RAM 不會包含在一致快照中。
