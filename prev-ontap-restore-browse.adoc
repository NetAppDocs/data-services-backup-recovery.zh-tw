...
summary: 使用 NetApp Backup and Recovery，透過瀏覽和還原來還原 ONTAP 資料。 
---
= 使用 NetApp Backup and Recovery 中的「瀏覽與還原」功能還原 ONTAP 資料
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您知道來源 Volume 名稱、來源系統、儲存設備虛擬機器和備份檔案日期，請使用「瀏覽和還原」功能。您可以從快照、複寫 Volume 或儲存在物件儲存中的備份還原 ONTAP 資料。
...
NOTE: 從 Azure 歸檔儲存設備還原資料到 StorageGRID 系統時，不支援高還原優先權。
...
您可以將 ONTAP 資料從位於二線系統（複製 Volume）或物件儲存（備份檔案）中的備份檔案還原到下列系統。
...
* 對於 Amazon S3，Console 代理程式可以部署在 AWS 中，也可以部署在本機。
* 對於 Azure Blob，Console 代理程式可以部署在 Azure 或本機。
* 對於 Google Cloud Storage，控制台代理程式必須部署在您的 Google Cloud Platform VPC 中
* 對於 StorageGRID，Console 代理程式必須部署在本機，無論是否具有網際網路存取。
* 對於 ONTAP S3，Console 代理程式可以部署在本機（無論是否具有網際網路存取）或雲端供應商環境中。


請注意，「內部部署 ONTAP 系統」包括 FAS、AFF 和 ONTAP Select 系統。
...
將雲端備份還原到執行 ONTAP 9.13.0 或更高版本的 Cloud Volumes ONTAP 系統，或還原到執行 ONTAP 9.14.1 的本機 ONTAP 系統時，您可以選擇執行「快速還原」作業。快速還原非常適合需要盡快提供 Volume 存取權限的災難恢復場景。快速還原會將備份檔案中的中繼資料還原到 Volume，而不是還原整個備份檔案。對於效能或延遲敏感型應用程式，不建議使用快速還原，且不支援對歸檔儲存設備中的備份執行快速還原作業。


NOTE: 只有當建立雲端備份的來源系統執行的是 ONTAP 9.12.1 或更高版本時，才支援對 FlexGroup Volume 進行快速還原。只有當來源系統執行的是 ONTAP 9.11.0 或更高版本時，才支援對 SnapLock Volume 進行快速還原。
...
+
「位置」欄位顯示備份檔案（快照）是位於「本機」（來源系統上的快照）、「輔助」（輔助 ONTAP 系統上的複寫 Volume）或「物件儲存」（物件儲存中的備份檔案）。選擇要還原的檔案。

. 選擇“下一步”。
+
請注意，如果您選擇物件儲存中的備份檔案，且該備份已啟用勒索軟體保護（如果您在備份原則中啟用了 DataLock 和勒索軟體保護），則在還原資料之前，系統會提示您對該備份檔案執行額外的勒索軟體掃描。我們建議您掃描該備份檔案是否含有勒索軟體。（存取備份檔案的內容會產生額外的雲端供應商出站流量費用。）

. 在「選擇目標」頁面中，選擇要恢復磁碟區的*系統*。
. 從物件儲存還原備份檔案時，如果您選擇本機ONTAP系統且尚未配置與物件儲存的叢集連接，系統將提示您輸入其他資訊：
+
** 從 Amazon S3 還原時，請在 ONTAP 叢集中選擇目的地 Volume 所在的 IPspace，輸入您所建立的使用者存取金鑰和秘密金鑰（以授予 ONTAP 叢集存取 S3 儲存桶的權限），並可選擇使用私人 VPC 端點進行安全的資料傳輸。如果使用私人 VPC 端點，其優先權高於任何已設定的 Proxy 連線。
...
. 您可以選擇啟用 *Prioritize this restore over existing job queue* 選項，以提高此還原作業的優先權。啟用後，其他正在執行的作業可能會暫停或延遲，以便此還原能夠順利進行。
. 選擇*恢復*。
+
返回「還原」儀表板，以便審查還原作業的進度。
...
+
如果備份檔案的勒索軟體保護處於作用中狀態（如果您在備份原則中啟用了 DataLock 和勒索軟體保護），系統會提示您在還原資料之前對備份檔案執行額外的勒索軟體掃描。我們建議您掃描備份檔案是否含有勒索軟體。（存取備份檔案的內容將產生額外的雲端供應商輸出流量費用。）
...
+
還原資料夾和檔案時，您可以選擇幾個還原位置。

+
** 當您選擇“選擇目標資料夾”時，如上所示：
+
*** 您可以選擇任意資料夾。
*** 您可以將滑鼠懸停在資料夾上，選取列尾以向下鑽取子資料夾，然後選取資料夾。


** 如果您選擇的目的地系統和 Volume 與來源資料夾或檔案位置相同，則可以選擇 *Maintain Source Folder Path* 將資料夾或檔案還原到它們在來源結構中存在的相同資料夾。所有相同的資料夾和子資料夾必須已存在；不會建立新資料夾。將檔案還原到原始位置時，您可以選擇覆蓋來源檔案或建立新檔案。
