...
[role="lead"]
使用 NetApp 備份與還原，透過「瀏覽與還原」還原 ONTAP 資料。還原前，請記下來源 Volume 名稱、來源系統和儲存設備虛擬機器，以及備份檔案日期。您可以從 Snapshot 快照技術、複寫的 Volume 或儲存在物件儲存中的備份還原 ONTAP 資料。
...
. 選擇“下一步”。
+
請注意，如果您選擇物件儲存中的備份檔案，且該備份啟用了勒索軟體防護（如果您在備份策略中啟用了 Datalock 和勒索軟體防護），則系統會提示您在還原資料之前對該備份檔案執行額外的勒索軟體掃描。我們建議您掃描該備份檔案是否有勒索軟體。（存取備份檔案的內容會產生額外的雲端供應商輸出費用。）
...
+
如果備份檔案啟用了勒索軟體防護（如果您在備份原則中啟用了 Datalock 和勒索軟體防護），則在還原資料之前，系統會提示您對備份檔案執行額外的勒索軟體掃描。我們建議您掃描備份檔案是否有勒索軟體。（存取備份檔案的內容會產生額外的雲端供應商出站流量費用。）
