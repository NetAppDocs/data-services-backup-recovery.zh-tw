...
---
= 提升 NetApp Backup and Recovery 本地部署中的搜尋與還原效能
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
將目錄索引儲存設備從叢集的預設儲存設備類別遷移至 ONTAP 支援的 NFS 共享，可顯著提升 NetApp Console 本地部署的目錄效能。索引目錄用於搜尋和還原功能。雖然目錄在預設儲存設備也能運作，但使用 ONTAP 支援的 NFS 可為搜尋和還原提供可量測的效能改善。
...
[discrete]
===== 在 NetApp Console 本機部署上裝載 ONTAP Volume，並從 pod 複製。
...
----
. 套用 YAML 對象，先套用 PV，再套用 PVC（PV 必須存在，PVC 的標籤選擇器才能連結到該 PV）。然後驗證 PVC 宣告是否已連結：
+
[source, shell]
----
