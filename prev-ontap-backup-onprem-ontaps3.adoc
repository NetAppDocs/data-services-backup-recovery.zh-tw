...
[role="lead"]
在 NetApp Backup and Recovery 中，您可以將主要內部部署 ONTAP 系統中的 Volume 資料備份到 ONTAP S3，也可以選擇備份到次要 ONTAP 系統以進行複寫。請使用此頁面準備您的環境並啟動備份。
...
. *複製*：如果選擇了*複製*，請設定以下選項：
+
** *複製目標*：選擇目的地系統和儲存設備虛擬機器。您也可以選擇目的地 Aggregate（或 FlexGroup Volume 的多個 Aggregate），以及將新增至複寫 Volume 名稱的前綴或後綴。
