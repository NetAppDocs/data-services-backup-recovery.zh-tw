---
sidebar: sidebar 
permalink: prev-ontap-backup-cvo-s3-compatible.html 
keywords: backing up, back up, backup, backup application data, Amazon Web services, AWS, NetApp Backup and Recovery, Oracle database, Microsoft SQL Server database, SAP HANA database 
summary: 使用 NetApp Backup and Recovery 保護您的 VMware 工作負載。 
---
= 使用 NetApp Backup and Recovery 將 Cloud Volumes ONTAP 資料備份到 S3 相容儲存設備
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
完成 NetApp Backup and Recovery 中的幾個步驟，即可開始將 Cloud Volumes ONTAP 系統中的磁碟區資料備份到 S3。
...
使用客戶管理的密鑰進行資料加密所需的信息:: 您可以在啟動精靈中選擇您自己的客戶管理金鑰進行資料加密，而不是使用預設的 S3 加密金鑰。在這種情況下，您需要事先設定好加密管理金鑰 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["了解如何使用自己的密鑰"^]。
...
所需的 AWS Cloud Volumes ONTAP權限:: 當您的 Cloud Volumes ONTAP 系統執行 ONTAP 9.12.1 或更高版本的軟體時，為該系統提供權限的 IAM 角色必須包含一組新的 S3 相容權限，專門用於最新版本的 NetApp Backup and Recovery https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Cloud Volumes ONTAP策略"^]。
+
--
...
+
如果您的備份的 S3 相容目的地在 *Systems* 頁面上以叢集的形式存在，您可以將該叢集拖曳到 S3 相容系統上以啟動設定精靈。
...
** 在備份和復原欄中選擇“磁碟區”。在「卷」標籤中，選擇「操作」。 image:icon-action.png["操作圖示"] 圖示選項，選擇“啟動 3-2-1 保護”，適用於單一磁碟區（尚未啟用複製或備份到物件儲存）。
...
** *備份*：將磁碟區備份到物件儲存。在選擇現有儲存桶或配置新儲存桶時，每個叢集最多可備份 6 個儲存桶中的磁碟區。
...
. *複製*：設定以下選項：
+
** *複製目標*：選擇目標系統和儲存虛擬機器。（可選）選擇目標聚合或多個聚合，以及要新增至複製磁碟區名稱的前綴或後綴。
** *複製策略*：選擇現有的複製策略或建立一個。
+
...
** *加密*：如果您建立了一個新的儲存桶，請輸入提供者提供給您的加密金鑰資訊。您可以選擇使用預設的 AWS 加密金鑰，或從您的 AWS 帳戶中選擇您自己的客戶管理金鑰來管理資料的加密。(https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["了解如何使用您自己的加密金鑰"^]）。
+
...
... 輸入策略的名稱。
... 選擇最多五個時間表，通常頻率不同。
... 對於備份到物件原則，請設定 Datalock 和 Ransomware 保護設定。如需 Datalock 和 Ransomware 保護的詳細資訊，請參閱link:prev-ontap-policy-object-options.html["備份到對象策略設置"]。
... 選擇“*創建*”。


** *匯出現有快照*：如果此系統中存在與您剛剛為此系統選擇的備份計畫標籤（例如，每日、每週等）相符的磁碟區的本機快照，則會顯示此附加提示。選取此框，即可將所有歷史快照複製到物件儲存作為備份文件，以確保您的磁碟區得到最全面的保護。
...
.步驟
. 在「審核」頁面中，審核您的選擇。
. （可選）選取此複選框可「自動修復本機快照、複製和備份中不符的標籤」。這將建立與快照、複製和備份策略中的標籤相符的標籤的快照。
. 選擇*啟動備份*。
