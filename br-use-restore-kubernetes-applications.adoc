...
---
= 使用 NetApp Backup and Recovery Web UI 還原 Kubernetes 應用程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在 NetApp Backup and Recovery 中還原受保護原則保護的應用程式。若要還原應用程式，您至少需要一個還原點，該還原點可以是本機快照、物件存放區備份或兩者兼有。您可以從本機、次要或物件存放區複本還原。
...
+
「還原資料」精靈隨即開啟，並顯示「一般設定」頁面。

. 選擇要從中還原的來源位置。
. 從*Cluster*清單中選擇目標群集。
. 選擇還原至原始命名空間或新命名空間。
. 如果您選擇還原到新的命名空間，請執行以下操作：
+
.. 請輸入要使用的目標命名空間或多個命名空間。
.. 請輸入目標應用程式名稱。
.. 您也可以選擇「不為已復原的資源建立應用程式」選項，以便在不建立應用程式自訂資源物件的情況下復原資源。這樣可以減少應用程式清單中不必要的條目。
...
.特定命名空間型應用程式資源
--
.. 選擇 *Selective resources* ，然後選擇使用規則篩選資源還是手動選擇資源。
+
[cols="1,1"]
|===
| 資源選擇方法 | 步驟 
...
. 如果您要還原使用 Trident Protect 建立的備份或快照，則可以選擇檢視 AppVault 用作還原作業儲存設備桶的詳細資訊。如果您的環境或 AppVault 狀態發生變更，請選擇 *Sync App Vault* 以重新整理詳細資訊。
+
...
NOTE: 在全域 link:br-start-configure.html#create-a-resource-transformation-template["設定"^] 區域建立資源轉換範本。
