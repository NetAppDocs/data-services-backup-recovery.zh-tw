...
[role="lead"]
根據您使用的是 NetApp Console 還是 NetApp Console （本地部署），您可以透過以下方式取得 NetApp Backup and Recovery 的授權：從雲端供應商購買隨用隨付 （PAYGO） 或年度市場訂閱的 *NetApp Intelligent Services*；或從 NetApp 購買自帶授權 （BYOL）。在系統上啟動 NetApp Backup and Recovery、建立正式作業資料的備份，以及將備份資料還原至正式作業系統，均需要有效的授權。
...
* 直接從 NetApp 購買授權，也稱為自帶授權 (BYOL)。此選項適用於 NetApp Console 和 Console 本機部署。
...
您可以使用 NetApp Console 或 Console 本機部署來管理 BYOL 授權。您可以從 Console 新增授權、更新現有授權以及檢視授權狀態。
...
超出許可容量將觸發 PAYGO 費率；如果沒有 PAYGO 訂閱，您將無法建立新的備份，但現有備份仍可恢復，只是服務不再有保障。請務必在許可證到期前續訂；過期的許可證將阻止建立新的備份並中斷服務。
