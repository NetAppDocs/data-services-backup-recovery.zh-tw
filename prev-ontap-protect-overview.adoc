...
* 如果您打算在還原磁碟區資料的過程中掃描備份檔案是否存在勒索軟體（如果您已為雲端備份啟用 Datalock 和勒索軟體保護），那麼您還將產生來自雲端供應商的額外輸出成本。
...
如果您的叢集使用的是 ONTAP 9.11.1 或更高版本，您可以保護物件儲存中的備份免受刪除和勒索軟體攻擊。每個備份原則都提供了一個 _Datalock 和 Ransomware 保護_ 部分，可以將其應用於備份檔案，並設定保護期限（即 _保留期_）。
