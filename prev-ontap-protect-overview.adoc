...
---
= 了解使用 NetApp Backup and Recovery 的 ONTAP Volume 保護
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 來保護和還原您的 ONTAP Volume 資料，並長期歸檔備份。您可以實作 3-2-1 策略，即在兩個不同的儲存設備系統上保留三份來源資料複本，並在雲端保留一份複本。


NOTE: 若要切換至NetApp Backup and Recovery工作負載，請參閱link:br-start-switch-ui.html["切換到不同的NetApp Backup and Recovery工作負載"]。

啟動後，備份與還原功能會建立區塊層級的遞增永久備份，這些備份儲存在另一個 ONTAP 叢集和雲端物件儲存設備中。除了來源 Volume 之外，您還將擁有：
...
NetApp 備份與還原使用 NetApp 的 SnapMirror 資料複寫技術，透過建立快照並將其傳輸至備份位置來保持備份同步。
...
* 使用兩個系統之間的預共用金鑰 (PSK) 設定原生 ONTAP 飛行中加密。
* 複製的資料是不可變的，直到您使其可寫入並可供使用為止。
* 如果傳輸失敗，複製可以自我修復。
* 與 https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^]相比，NetApp 備份與還原中的複寫包含下列功能：
+
...
* 掃描備份檔案是否有勒索軟體攻擊，並自動移除或替換受感染的備份。
...
NetApp Backup and Recovery 可讓您將 ONTAP Volume 從下列來源系統備份到下列二線系統，以及公有雲和私有雲供應商中的物件儲存。Snapshot 位於來源系統上。


NOTE: 目前，Wasabi 是唯一獲得認證的 S3 相容物件儲存提供者。其他 S3 相容提供者尚未通過驗證。
...
| Google 的Cloud Volumes ONTAP  a| 
* Google 的Cloud Volumes ONTAP
* 本地ONTAP系統
...
| S3 相容物件儲存設備 | 本地ONTAP系統 | 本地ONTAP系統 
|===

NOTE: 文中提到的「內部部署 ONTAP 系統」包括 FAS、AFF 和 ONTAP Select 系統。
...
* 如果您打算在還原 Volume 資料的過程中掃描備份檔案是否存在勒索軟體（如果您已為雲端備份啟用 DataLock 和勒索軟體保護），那麼您還將產生來自雲端供應商的額外輸出費用。
...
NOTE: 套用於 Volume 的快照原則必須包含您在複寫原則和備份到物件原則中使用的標籤之一。如果找不到符合的標籤，則不會建立備份檔案。例如，如果您想要建立「每週」複寫 Volume 和備份檔案，則必須使用建立「每週」快照的快照原則。
...
如果您的叢集使用 ONTAP 9.11.1 或更高版本，您可以保護物件儲存中的備份，使其免受刪除和勒索軟體攻擊。每個備份原則都提供了一個用於 _DataLock 和勒索軟體保護_ 的區段，可在特定期間內套用至您的備份檔案，即 _保留期間_。
...
目前，當備份目的地為 Amazon S3、Azure Blob 或 StorageGRID 時，此功能受到支援。其他儲存設備供應商目的地將在未來的版本中新增。
...
+
如果您的本機叢集使用的是 ONTAP 9.12.1 或更高版本，而您的 StorageGRID 系統使用的是 11.4 或更高版本，則可以在一定天數後將較舊的備份檔案歸檔到公有雲歸檔儲存設備。目前支援 AWS S3 Glacier/S3 Glacier Deep Archive 或 Azure Archive 儲存設備層。link:prev-ontap-backup-onprem-storagegrid.html["了解有關從StorageGRID歸檔備份檔案的更多信息"]。



請參閱link:prev-ontap-policy-object-options.html["舊備份檔案歸檔的工作原理"]。
