...
SnapCenter 不會將儲存叢集或儲存 VM 對應到資源的主機，但 NetApp Backup and Recovery 支援此功能。在 NetApp Backup and Recovery 預覽版中，內部部署 ONTAP 叢集或儲存 VM 不會對應到主機。此外，NetApp Console 不支援儲存 VM。
