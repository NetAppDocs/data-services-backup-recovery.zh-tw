...
[role="lead"]
在 NetApp Backup and Recovery 中使用備份至物件原則，為您的內部部署 ONTAP 和 Cloud Volumes ONTAP 系統定義排程、保留、DataLock、勒索軟體保護和歸檔設定。
...
每項備份原則都提供一個「標籤和保留」部分，您可以將其套用至備份檔案。請注意，套用至 Volume 的快照原則必須是 NetApp Backup and Recovery 所識別的原則之一，否則將不會建立備份檔案。
...
這兩項功能可保護您的備份檔案，確保您在備份遭受勒索軟體攻擊時，始終擁有有效的備份檔案以還原資料。此外，這些功能也有助於滿足某些法規要求，例如備份需要鎖定並保留特定一段時間。啟用「*DataLock 和勒索軟體保護*」選項後，作為 NetApp Backup and Recovery 啟動一部分而佈建的雲端儲存桶將啟用物件鎖定和物件版本控制功能。
...
借助此功能，您可以鎖定透過SnapMirror複製到雲端的雲端快照，還可以啟用該功能來偵測勒索軟體攻擊並恢復物件儲存上快照的一致副本。此功能在 AWS、Azure、Google Cloud Platform 和StorageGRID上均支援。
...
. 建立原則時，展開 *DataLock 和勒索軟體保護* 區段。
. 選擇下列選項之一：
+
** *無*：DataLock 保護和勒索軟體保護均已停用。
** *已解鎖*：DataLock 保護和勒索軟體保護已啟用。擁有特定權限的使用者可以在保留期內覆寫或刪除受保護的備份檔案。
** *已鎖定*：DataLock 保護和勒索軟體保護已啟用。在保留期間內，任何使用者都無法覆寫或刪除受保護的備份檔案。這完全符合法規遵循要求。
...
| AWS 中的Cloud Volumes ONTAP | 亞馬遜 S3 


| Azure 中的Cloud Volumes ONTAP | Azure Blob 


| Google Cloud 中的Cloud Volumes ONTAP | Google 雲端儲存 


| 本地ONTAP系統 | Amazon S3 Azure Blob Google Cloud NetApp StorageGRID 
|===
...
使用 AWS、Azure 或 Google Cloud Storage 時，您可以將較舊的備份檔案在一定天數後移至成本較低的歸檔儲存設備層級或存取分層。您也可以選擇立即將備份檔案傳送到歸檔儲存設備，而無需寫入標準雲端儲存。只需將「歸檔天數」設定為 *0*，即可將備份檔案直接傳送到歸檔儲存設備。這對於很少需要存取雲端備份資料的使用者或正在替換磁帶備份解決方案的使用者尤其有用。
...
+
** 對於 AWS，您可以將備份分層儲存到 AWS _S3 Glacier_ 或 _S3 Glacier Deep Archive_ 儲存中。link:prev-reference-aws-archive-storage-tiers.html["了解有關 AWS 檔案儲存的更多信息"^]。
** 對於 Azure，您可以將較舊的備份分層儲存到 _Azure Archive_ 儲存體中。link:prev-reference-azure-archive-storage-tiers.html["了解有關 Azure 檔案儲存的更多信息"^]。
