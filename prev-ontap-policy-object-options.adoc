...
這兩項功能都能保護您的備份檔案，確保您始終擁有有效的備份檔案，以便在備份遭受勒索軟體攻擊時能夠恢復資料。此外，它們還有助於滿足某些法規要求，例如備份需要鎖定並保留一段時間。啟用 *Datalock and Ransomware protection* 選項後，作為 NetApp Backup and Recovery 啟動的一部分而佈建的雲端儲存貯體將啟用物件鎖定和物件版本控制功能。
...
| Google Cloud 中的Cloud Volumes ONTAP | Google雲 
