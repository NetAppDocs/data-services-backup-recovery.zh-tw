...
[role="lead"]
使用 NetApp Backup and Recovery 中的 SnapMirror to Cloud Resync，可在您使用 SnapMirror 邏輯複寫（LRSE）移轉 Volume 後，保持現有雲端備份可用。此功能可讓備份繼續進行而無需重新建立基準，並自 ONTAP 9.16.1 起提供。

SnapMirror to Cloud Resync 支援 FlexVol® 和 FlexGroup 移轉，並協助您在混合雲和多雲環境中維持備份連續性。
...
假設您有一個名為 Vol1a 的內部部署 Volume。該 Volume 有三個快照：S1、S2 和 S3。這些快照就是還原點。Vol1a 使用 SnapMirror to Cloud （SM-C） 備份到雲端，但物件存放區中只有 S1 和 S2。
...
. 檢查 Volume 資格。系統會檢查 Volume 是否符合重新同步的條件。如果某個 Volume 不符合條件，則表示未符合通用 Snapshot 標準。
+
