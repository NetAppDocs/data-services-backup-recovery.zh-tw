...
NOTE: 請勿使用SnapMirror「Vault」類型的策略將磁碟區從舊的 ONTAP 叢集複製到新的 ONTAP 叢集，或在級聯架構設定中複製。Vault 策略會在傳輸過程中變更快照的版本 UUID。快照的版本 UUID 用於確定來源和目標之間是否存在通用快照。
...
. 選擇*提交*開始驗證程序。
+
NetApp Backup and Recovery 會驗證每個選擇重新同步的磁碟區是否與物件儲存區共享相同的快照。具體來說，儲存在物件儲存區中的最新快照也必須存在於新複製磁碟區的本地快照中。此檢查可確保這些磁碟區已準備好進行 SnapMirror to Cloud Resync 作業。

. 查看驗證結果，包括新的來源磁碟區名稱和每個磁碟區的重新同步狀態。
. 檢查磁碟區資格。系統會檢查磁碟區是否符合重新同步的條件。如果某個磁碟區不符合條件，則表示未建立通用 Snapshot 標準。
+
