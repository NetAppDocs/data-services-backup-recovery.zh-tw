...
---
= 使用 NetApp Backup and Recovery 中的搜尋和還原功能來還原 ONTAP 資料
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
當您不知道確切的系統名稱、Volume 或檔案名稱時，可以使用「搜尋和還原」功能從 ONTAP 備份中還原 Volume、資料夾或檔案。您可以搜尋本機快照、複製的 Volume 和物件儲存備份。
...
* 當您還原已啟用 SnapLock 的磁碟區時，只有在目的地儲存設備已啟用 WORM 的情況下，還原的磁碟區才會保留 SnapLock 保護。如果還原到非 WORM 目的地，SnapLock 狀態將從還原的磁碟區中移除。


====
在開始之前，您應該對要還原的 Volume 或檔案的名稱或位置有所了解。
...
| StorageGRID | 本機ONTAP系統Cloud Volumes ONTAP | 本地ONTAP系統 
...
** 您必須在儲存虛擬機器上設定 IPv4 NFS 資料 LIF，並為該磁碟區配置 NFS 匯出。Search & Restore 同時支援 NFS 和 SMB/CIFS 磁碟區。
** SnapDiff RPC 伺服器必須在儲存設備虛擬機器上啟動。在系統上啟用索引時，Console 會自動執行此操作。（SnapDiff 是一種能夠快速識別 Snapshot 快照技術之間檔案和目錄差異的技術。）
...
* Azure 需求：
+
** 您必須將 Azure Synapse Analytics 資源供應商（名為「Microsoft.Synapse」）註冊到您的訂閱中 https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["了解如何為您的訂閱註冊此資源提供程序"^]。您必須是訂閱的*擁有者*或*參與者*才能註冊資源供應商。
...
NOTE: NetApp Backup and Recovery 不會對建立時間戳記與相同磁碟區的另一個 Snapshot 完全相同的 Snapshot 進行索引。
...
首先，裝載一個分隔的 Volume 來存放目錄檔案。這有助於防止快照目錄檔案過大時導致資料遺失。並非每個叢集都需要這樣做；您可以裝載環境中任何叢集的任意一個 Volume。如果不這樣做，索引可能無法正常運作。
...
.. 在「篩選依據」區域中，選擇篩選條件。例如，您可以選擇資料所在的系統和檔案類型，例如 .JPEG 檔案。如果您只想在物件儲存中的可用快照或備份檔案中搜尋，也可以選擇備份位置類型。
...
+
請注意，搜尋結果會顯示包含您搜尋檔案的本機 Volume 快照和遠端複製 Volume。您可以選擇從雲端備份檔案、快照或複製 Volume 還原。
...
+
*** 從 Amazon S3 還原時，請在 ONTAP 叢集中選擇目的地 Volume 所在的 IPspace，輸入您所建立的用於授予 ONTAP 叢集存取 S3 儲存桶權限的使用者存取金鑰和秘密金鑰，並可選擇使用私人 VPC 端點進行安全的資料傳輸。如果使用私人 VPC 端點，其優先順序高於任何已設定的 Proxy 連線。link:prev-ontap-backup-onprem-aws.html["查看有關這些要求的詳細信息"].
