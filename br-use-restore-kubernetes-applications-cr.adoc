...
[role="lead"]
使用自訂資源從快照或備份還原應用程式。將應用程式還原到同一叢集時，從現有快照還原通常速度更快。
...
* 您可以使用進階設定來自訂還原操作。若要深入瞭解，請參閱 link:br-use-kubernetes-advanced-restore-settings.html["NetApp Backup and Recovery 中 Kubernetes 的進階還原設定"]。
...
* 使用 CR 還原到新命名空間時，必須先手動建立目的地命名空間，然後再套用 CR。備份與恢復功能僅在使用 CLI 時才會自動建立命名空間。
...
NOTE: 如果 BackupRestore CR 不可用，您可以使用步驟 2 中的命令檢視備份內容。
