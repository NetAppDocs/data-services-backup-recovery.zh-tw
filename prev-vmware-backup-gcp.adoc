...
---
= 使用NetApp Backup and Recovery將資料儲存備份到 Google Cloud Platform
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以將SnapCenter Plug-in for VMware vSphere與NetApp Backup and Recovery集成，將一個或多個資料儲存區備份到 Google Cloud Platform。這將有助於 VM 管理員輕鬆快速地備份和存檔數據，以提高儲存效率並加速雲端轉換。
