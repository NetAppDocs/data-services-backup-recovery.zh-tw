...
[role="lead"]
在 NetApp 備份與還原中，您可以將內部部署 ONTAP 系統中的 Volume 資料備份到次要儲存設備和 Azure Blob。請使用此頁面準備您的環境並啟動備份。
...
+
NetApp Backup and Recovery 在 Azure 政府區域中受到支援，前提是 Console 代理程式部署在雲端，而非安裝在您的本機環境中。此外，您必須從 Azure Marketplace 部署 Console 代理程式。您無法從 NetApp Console SaaS 網站在政府區域部署 Console 代理程式。
...
+
** 對於 NetApp Backup and Recovery PAYGO 授權，您需要訂閱 https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Azure 市場提供的NetApp Console"^]。NetApp Backup and Recovery 的計費透過此訂閱進行。
...
* 如果您使用的 IPspace 與預設 IPspace 不同，則可能需要建立靜態路由才能存取物件儲存設備。
...
+
請注意，NetApp Backup and Recovery 支援 _Azure 存取原則_ 作為權限模型。目前不支援 _Azure 角色型存取控制_（Azure RBAC）權限模型。
...
+
** 在 Console 的 *Systems* 頁面中，選擇系統，然後在右側面板的備份與還原服務旁選擇 *Enable > Backup Volumes*。
+
...
+
精靈的「簡介」頁面顯示了保護選項，包括本機快照、複寫和備份。如果您在此步驟中選擇了第二個選項，則會顯示「定義備份策略」頁面，並已選取一個 Volume。
...
NOTE: 如果您選擇 *Bring your own bucket* ，請確保您建立的用作備份目標的容器與 Azure 儲存體帳戶同名。
...
.步驟
. 在「審核」頁面中，審核您的選擇。
. 您可以選擇勾選「自動將快照原則標籤與複寫和備份原則標籤同步」複選框。這將建立標籤與複寫和備份原則中的標籤相符的快照。
. 選擇*啟動備份*。
