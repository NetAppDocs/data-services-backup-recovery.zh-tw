...
** 如果在 SnapMirror 目的地 Volume 上建立快照的本機副本（無論 SnapMirror 使用何種標籤），這些快照都不會作為備份移至雲端。您必須在來源 DP Volume 上建立具有所需標籤的快照原則，以便 NetApp Backup and Recovery 可以備份這些快照。
...
+
** 僅支援從ONTAP輔助節點進行備份。
** 套用至 Volume 的快照原則必須是 NetApp Backup and Recovery 所識別的原則之一，包括每日、每週、每月等。預設的「sm_created」原則（用於*鏡像所有快照*）不被識別，因此 DP Volume 不會顯示在可備份 Volume 的清單中。
