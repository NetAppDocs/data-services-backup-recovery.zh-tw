...
summary: 在 NetApp Backup and Recovery 中建立和管理 Oracle Database 工作負載的保護群組。 
---
= 使用 NetApp Backup and Recovery 為 Oracle Database 工作負載建立及管理保護群組
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
建立保護群組以管理 Oracle 資料庫資源的備份作業。保護群組是將要一起保護的資源（例如資料庫）進行邏輯分組。

您可以執行與保護群組相關的以下任務：

* 建立保護組。
* 查看保護詳情。
* 立即備份保護群組。請參閱link:br-use-oracle-backup.html["立即備份 Oracle Database 工作負載"]。
* 刪除保護組。
