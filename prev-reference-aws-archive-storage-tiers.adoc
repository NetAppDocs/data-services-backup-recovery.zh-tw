...
[role="lead"]
使用此資訊為 NetApp Backup and Recovery 備份選擇 AWS S3 歸檔分層。您也可以審查已歸檔備份的還原成本和擷取時間。
...
備份檔案最初建立時，會儲存在 S3 _Standard_ 儲存設備。此分層針對不經常存取的資料進行最佳化，但仍允許立即存取。30 天後，備份檔案將轉換至 S3 _Standard-Infrequent Access_ 儲存類別，以節省成本。
