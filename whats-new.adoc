...
* NetApp Backup and Recovery現已正式提供 VMware 工作負載支援 (GA)。
* 現在您可以還原客戶作業系統檔案和資料夾。


https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-vmware-protect-overview.html["了解更多關於使用NetApp Backup and Recovery恢復 VMware 虛擬機器的信息"]。
...
* 現在您可以將 Hyper-V VM 備份和快照還原到備用位置。如果您想管理不同 Hyper-V 主機上的虛擬機器版本，這將非常有用。
* NetApp Backup and Recovery現在支援由 System Center Virtual Machine Manager (SCVMM) 配置並託管在 CIFS 共用上的 Hyper-V 虛擬機器。
* 現在您可以編輯保護群組了。
...
KVM 工作負載預覽版現在支援與 Apache CloudStack 管理平台集成，使您能夠保護由 Apache CloudStack 管理的 KVM 主機和虛擬機器。
