...
[role="lead"]
在保護運行於 ONTAP 系統上的 VMware 資料儲存和虛擬機器之前，您需要在 NetApp Backup and Recovery 中探索它們。如果您已安裝 SnapCenter Plug-in for VMware vSphere，也可以匯入其備份資料和原則。
...
如果您尚未安裝 SnapCenter Plug-in for VMware，請新增 vCenter 相關資訊，以便 NetApp Backup and Recovery 能夠探索工作負載。在每個 Console 代理程式中，選擇要探索工作負載的系統。
...
如果您已安裝 SnapCenter Plug-in for VMware，請依照下列步驟將 SnapCenter Plug-in 資源匯入 NetApp Backup and Recovery。Console 會從 Plug-in 中探索 ESXi 主機、資料存放區、虛擬機器和排程，因此您無需重新建立這些資訊。
...
NOTE: 如果 SnapCenter 外掛程式已註冊，您可以更新現有的註冊資訊。
